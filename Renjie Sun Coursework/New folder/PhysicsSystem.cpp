...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if (physA->GetInverseMass() == 0.f && physB->GetInverseMass() == 0.f) // exit early for static object collisions
		return;
	...
	//if (a.GetName() == "Ball" || b.GetName() == "Ball") {
		//std::cout << impulseForce << std::endl;
		//std::cout << "fullImpulse on Ball: " << fullImpulse << " impulse normal: " << p.normal << std::endl;
	//}
...
void PhysicsSystem::IntegrateAccel(float dt) {
	...
	for (auto i = first; i != last; ++i) {		PhysicsObject * object = (*i)->GetPhysicsObject();
		...
		//if ((*i)->GetName() == "Ball") {
		//	std::cout << "force on Ball: " << force << " acell " << accel << std::endl;
		//}
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	for (auto i = first; i != last; ++i) {
		...
		transform.SetWorldPosition(position);//不知道是什么
		// Linear Damping
